#!/usr/bin/env python3
"""
MICROCORE (MCX) COMPLETE NODE v21.0 - FINAL
...
# ==================== RATE LIMITER ====================
class RateLimiter:
    """Rate limiter for WebSocket connections to prevent spam/abuse"""
...
class P2PNode:
    ...
    async def _handle(self, r, w):
        ...
        try:
            ...
            if msg_len > 10_000_000:
                self._ban_peer(addr_str, "Message too large")
                w.close()
                return
    ...
    def _ban_peer(self, addr, reason):
        ...
        if addr in self.peers:
            del self.peers[addr]
        logger.warning(f"[P2P] Banned {addr}: {reason}")
...
class MicroCoreNetwork:
    ...
    def _load(self):
        ...
        logger.info(f"[LOAD] {len(self.blocks)} blocks, {len(self.miners)} miners, {len(self.nodes)} nodes")
...
# ==================== PAYMENT PROCESSING ====================
    ...
    def confirm_stripe_payment(self, payment_intent_id):
        ...
        logger.info(f"[STRIPE] Credited {mcx_amount} MCX to {wallet}")
    ...
    async def _handle_ws_message(self, websocket, data, msg_type, client_ip):
        ...
        elif msg_type == "confirm_stripe_payment":
...
# ==================== BLOCK PRODUCTION WITH REDISTRIBUTION ====================
    ...
    def slash_miner(self, vid, reason, block_id=-1):
        ...
        if m.slashes >= BAN_THRESHOLD:
            m.active = False
            logger.warning(f"[BAN] {m.username} banned after {BAN_THRESHOLD} slashes")
...
class MicroCoreServer:
    ...
    async def status_reporter_loop(self):
        while not self._shutdown:
            try:
                ...
                logger.info(f"MCX Price: ${MCX_PRICE_USD:.4f} USD")
    ...
    async def health_check_loop(self):
        """Simple health check endpoint via HTTP"""
        while not self._shutdown:
            try:
                ...
                # Could expose this via a simple HTTP server if needed
    ...
    async def run(self):
        ...
        logger.info(f"📊 STRIPE INTEGRATION: Real credit card payments")
